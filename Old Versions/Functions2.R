...
fit_ds_model <- function(region,
  ...
  Region_Area <- sum(region@area, na.rm = TRUE)
  ...
  # calculate effort and observation counts 
  obs_counts <- obsdata |> 
  ...
  if (transect_type == "line") {
    ...
    se_P2 <- p2$dht$individuals[3]$N[3]$se
    ...
    se_P3 <- p3$dht$individuals[3]$N[3]$se
    ...
    #   
    #   se_D_R2 = delta_P2$se_D,
    #   se_D_R3 = delta_P3$se_D,
    # )
  ...
  list(
    ds_model = if (transect_type == "line") m1 else p2,
    N_hat = N_hat,
    se_m1 = se_ds,
    sigma_hat = sigma_hat,
    analytical_variances = analytical_variances # Add this line!
  )
}
