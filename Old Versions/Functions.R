...
get_fit <- function(dist_data,
  ...
  if (transect_type == "line") {
    ...
    get_boxlet_se <- function(dsm_model, m1_model, region, N_hat, 
                          spacing = POINT_SPACING, 
                          truncation = TRUNCATION_DISTANCE, 
                          grid_res = 100) {
  
  # STEP 1: Tessellate the Region (The Boxlets)
  # ---------------------------------------------------------
  ...
  # STEP 2: Estimate Boxlet Probabilities (p_j)
  # ---------------------------------------------------------
  ...
  # Extract Average Detection Probability (P_a) from the distance model
  P_a <- as.numeric(m1_model$dht$individuals$summary$p)
  
  # STEP 3: Simulate the Grid Shifts (b)
  # ---------------------------------------------------------
  ...
  # STEP 4: Calculate Q(b) for Each Shift
  # ---------------------------------------------------------
  for (i in 1:B) {
  ...
  # STEP 5: Apply the Multinomial Equation
  # ---------------------------------------------------------
  ...
  se_n <- sqrt(max(0, var_n)) # Ensure non-negative variance before rooting
