...
if (exists("pred_grid") && nrow(obsdata) > 0) {
  ggplot(pred_grid) +
    geom_sf(aes(fill = density), color = NA) +
...
if (exists("pred_grid") && !is.na(N_hat) && N_hat > 0) {
  ...
  bootstrap_results <- data.frame(replicate = seq_len(bootstrap_reps), N_hat = N_hat_results)
...
if (exists("bootstrap_results")) {
  ...
  p_hist <- ggplot(bootstrap_results, aes(x = N_hat)) +
    ...
    scale_color_manual(name = "Legend", values = c("Mean Boot N_hat" = "red", 
                                                   "True Model N_hat" = "darkgreen")) +
    theme_minimal() +
  ...
  print("Bootstrap sequence did not run (likely missing observations or surface data).")
}
```
...
```{r}
# Safely extract values from a list to avoid NULL errors
...
if (points_or_lines == "line") {
  ...
  boxlet_est   <- safe_extract(analytical_variances, "se_N_boxlet")
}

bootstrap_est <- if (exists("empirical_SE")) empirical_SE else NA_real_
...
variance_comparison <- tibble(
  ...
  `SE (N_hat)` = c(
    R2_est, R3_est, S1_est, S2_est, O1_est, O2_est, 
    P2_est, P3_est, striplet_est, boxlet_est, bootstrap_est
  )
) |> 
